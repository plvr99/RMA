...
class MainActivity : AppCompatActivity(){
    ...
    private val mOnNavigationItemSelectedListener = BottomNavigationView.OnNavigationItemSelectedListener { item ->
        when (item.itemId) {
            ...
            R.id.predajKviz ->{
                ...
                val string = "Završili ste kviz ${frag.nazivKviza} sa tačnosti ${frag.procenatTacnosti()}"
                val poruka = FragmentPoruka.newInstance(string)
                //sacuvaj odgovore
                println("ODgovor koji treba sacuvat" + frag.odgovor)
    ...
    private fun openFragment(fragment: Fragment, tag: String) {
        ...
        transaction.commit()
        println(" frag size" +supportFragmentManager.fragments.size )
        println("backstack size" +supportFragmentManager.backStackEntryCount)
    }
    ...
    private fun disableSelectionOdgovora(odgovor: Odgovor){
        ...
        for (i in 0 until odgovor.pitanja.size) odgovor.pitanja.get(i).odgovorenaPozicija=-2
    }
    ...
    //todo dodati pitanja u repositiory, popraviti rotiranje(mozda), popraviti pamcenje spinnera, dodati testove,
}
